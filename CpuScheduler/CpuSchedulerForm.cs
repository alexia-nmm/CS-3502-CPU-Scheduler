...
/// If a new process arrives with a shorter remaining time than the current one,
...
private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
{
    ...
    int[] remaining = new int[n];          // remaining burst time per process
    int[] start     = new int[n];          // first time each process gets the CPU
    bool[] started  = new bool[n];         // has this process started yet?
    ...
    int lastArrival    = processes.Max(p => p.ArrivalTime);
    int simulationEnd  = lastArrival + totalBurst + 1; // safe upper bound

    while (completed < n && currentTime <= simulationEnd)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (processes[i].ArrivalTime <= currentTime && remaining[i] > 0)
            {
                if (remaining[i] < minRemaining ||
                   (remaining[i] == minRemaining && processes[i].ArrivalTime < processes[chosen].ArrivalTime))
                {
        ...
        if (remaining[chosen] == 0)
        {
            ...
            int turnaround     = finishTime - processes[chosen].ArrivalTime;
            int waiting        = turnaround  - processes[chosen].BurstTime;

            results.Add(new SchedulingResult
            {
                ProcessID     = processes[chosen].ProcessID,
                ArrivalTime   = processes[chosen].ArrivalTime,
                BurstTime     = processes[chosen].BurstTime,
                StartTime     = start[chosen],
                FinishTime    = finishTime,
                WaitingTime   = waiting,
...
/// When the CPU is free, pick the process with the highest Response Ratio:
///     RR = (WaitingTime + BurstTime) / BurstTime
/// This naturally favours short jobs but also rewards processes that have
/// been waiting a long time, preventing starvation.
/// </summary>
private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
{
    var results           = new List<SchedulingResult>();
    var remaining         = processes.ToList();   // processes not yet scheduled
    int currentTime       = 0;
    ...
    while (remaining.Count > 0)
    {
        ...
        {
            // Jump forward to the next arrival
            currentTime = remaining.Min(p => p.ArrivalTime);
            continue;
        }

        // Calculate response ratio for each available process and pick the highest
        ...
        foreach (var p in available)
        {
            ...
            double responseRatio   = (waitingTime + p.BurstTime) / (double)p.BurstTime;

            if (responseRatio > bestRR)
            {
                bestRR = responseRatio;
                chosen = p;
            }
        }
        ...
        int finishTime     = startTime     + chosen.BurstTime;
        int turnaround     = finishTime    - chosen.ArrivalTime;
        int waiting        = turnaround    - chosen.BurstTime;
    ...
    return results;
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            {
                var item = new ListViewItem(r.ProcessID);
                item.SubItems.Add(r.ArrivalTime.ToString());
                item.SubItems.Add(r.BurstTime.ToString());
                item.SubItems.Add(r.StartTime.ToString());
                item.SubItems.Add(r.FinishTime.ToString());
                item.SubItems.Add(r.WaitingTime.ToString());
                item.SubItems.Add(r.TurnaroundTime.ToString());
            ...
            int    totalElapsed = results.Max(r => r.FinishTime) - results.Min(r => r.ArrivalTime);
            double cpuUtil      = totalElapsed > 0 ? totalBurst / (double)totalElapsed * 100.0 : 100.0;
            double throughput   = totalElapsed > 0 ? results.Count / (double)totalElapsed : 0.0;
            ...
            // --- Summary header ---
            var header = new ListViewItem("── PERFORMANCE METRICS ──");
            header.SubItems.Add(algorithmName);
            header.SubItems.Add(""); header.SubItems.Add("");
            header.SubItems.Add(""); header.SubItems.Add(""); header.SubItems.Add("");
            listView1.Items.Add(header);

            void AddMetric(string name, string value, string note)
            {
                var row = new ListViewItem(name);
                row.SubItems.Add(value);
                row.SubItems.Add(note);
                row.SubItems.Add(""); row.SubItems.Add("");
                row.SubItems.Add(""); row.SubItems.Add("");
                listView1.Items.Add(row);
            }

            AddMetric("Avg Waiting Time (AWT)",   $"{awt:F2} units",            "Lower is better");
            AddMetric("Avg Turnaround Time (ATT)", $"{att:F2} units",            "Lower is better");
            AddMetric("CPU Utilization",           $"{cpuUtil:F1}%",             "Higher is better");
            AddMetric("Throughput",                $"{throughput:F4} proc/unit", "Higher is better");
            AddMetric("Avg Response Time",         $"{avgResponse:F2} units",    "Lower is better");
            AddMetric("Processes Completed",       $"{results.Count}",           "");
            AddMetric("Total Elapsed Time",        $"{totalElapsed} units",      "");
        }
        ...
        private void ApplyLightTheme()
        {
            ...
            btnHRRN.BackColor = Color.MistyRose;
